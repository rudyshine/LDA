...
import pandas as pd
import urllib, re, random, time, sys
...
ws1 = urllib.request.urlopen(url1).read().decode('utf-8').encode(sysEncode)
...
for page in range(1, pages + 1)[:2]:
    ...
    ws2 = urllib.request.urlopen(url2).read().decode('utf-8').encode(sysEncode)
    # get id comment name
    groups = re.findall(
        r'<a href="//item.jd.com/(\d+).html#comment" target="_blank">.*?<em>(\d+)</em>.*?</a>[\s\S]*?target="_blank">(.*?)</a>',
        ws2)
    ...
    for id in l_id_copy:
        ...
        ws3 = urllib.request.urlopen(url3).read().decode('utf-8').encode(sysEncode)
...
print
len(l_id), len(l_name), len(l_price)
