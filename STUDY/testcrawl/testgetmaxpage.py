...
def getmaxpage():
    ...
    html0=str(html0)
    file = codecs.open(filename+"page.txt", "w")
    file.write(html0)
    file.close()
    html0 = codecs.open(filename+'page.txt', 'r').read()
...
def crawlpage(ran_num):
    ...
    html = codecs.open(filename+"page.txt", 'r').read()
    ...
    table.to_csv(filename+'jd_table.csv', mode='a', header=False)
    print("存完....")
    ...
    sheet_table = rent_info[filename+'jd_table']  # 创建表单
